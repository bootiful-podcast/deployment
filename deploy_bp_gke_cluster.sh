...
function deploy_new_gke_cluster(){
  ...
  gcloud --quiet  beta container --project $GKE_PROJECT clusters create "${GKE_CLUSTER_NAME}" \
     --zone "us-central1-c" --no-enable-basic-auth --cluster-version "1.16.13-gke.401" \
     --machine-type "c2-standard-4" --image-type "COS" --disk-type "pd-standard" --disk-size "100" \
     ...
     --num-nodes "3" --enable-stackdriver-kubernetes --enable-ip-alias --network "projects/${GKE_PROJECT}/global/networks/default" \
